...
```{r}
navn_var <- unique(df$statistikkvariabel) # lager en liste med statestikkvariablene

df_vekter <- df %>% 
  filter(statistikkvariabel == navn_var[2]) # filtrer slik at jeg bare velger konsumprisindeks i vekter

df_vekter <- df_vekter %>% 
  mutate(value = value/1000) # jeg bruker mutate for å dele value på 1000

sjekk <- df_vekter %>% # jeg lager sjekk, for å gruppere data etter dato.
  group_by(dato) %>%
...
sjekk <- na.omit(sjekk) # fjerner NA verdier
...
df_2 <- filter(df,statistikkvariabel == navn_var[1]) #lager et nytt dataframe og filtrerer for konsumprisindeksen


df_2 %>% 
  filter(konsumgruppe == 	"Elektrisitet inkludert nettleie") %>% #Filtrerer Elektrisitet inkludert nettleie
  ggplot() + #Lager en figur med ggplot
  geom_line(aes(x = dato, y=value, color="Elektrisitet inkludert nettleie")) + # Jeg velger å bruke et linjediagram
  theme(legend.position="bottom")+ # setter legend nederst på figuren
  xlab("Verdien med hensyn til KPI") + #setter x laben
  ylab("Tid (År)") + # setter y label
  ggtitle("Vekstbidrag for elektrisitet inkludert nettleie") # setter tittel
```

Her kan vi se ved å sette opp konsumprisindeksen på y aksen, akkurat hvor mye prisene har steget. På x- aksen har vi tid i årstall fra 2011 til 2022. Vekstbidraget har steget fram til 2019, hvor den så sank, før den så steg betydlig i 2021.
...
*Svært høye strømpriser bidrar til å øke prisveksten for samfunnet. Når strømprisene øker, gjør dette det mer kostbart for bedrifter å drive produksjonen i en butikk, fabrikk, men også transport siden vi nå går over til elektriske kjøretøy. Dette må bedriftene kompensere ved å øke prisene de tar for sine varer og tjenester.*

*Dette påvirker jo også energiregningen for privatpersoner. Derfor får befolkningen en lavere kjøpekraft, siden strøm er en nødvendighetsvare.*
